...
import com.zdjc.zdjcyun.mvp.presenter.ProjectImagesPresenter;
...
public class ProjectImagesPresenterImpl extends BaseNetControl implements ProjectImagesPresenter {

    @Override
    public void getQueryImageNames(RequestCallBack callBack, Map<String, String> params, int tag) {
        /**
         * 请求前
         */
        callBack.beforeRequest(tag);
        HttpRequestImpl.getInstance().queryImageNames(params)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<ImageListEntity>() {
                    @Override
                    ...
                    @Override
                    public void onNext(ImageListEntity value) {
                        /**
                         * 这里是回掉成功的
                         */
                        if(value.getCode()==0){
