...
public abstract class RecycleViewGridDecoration extends RecyclerView.ItemDecoration {

    private Paint mPaint;
    private int   lineWidth;//px 分割线宽

    public RecycleViewGridDecoration(Context context, float lineWidthDp, @ColorInt int colorRGB) {
    ...
    public RecycleViewGridDecoration(Context context, int lineWidthDp, @ColorInt int colorRGB) {
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        //可根据实际项目需要修改成合适的值
        int left = sideOffsetBooleans[0] ? lineWidth/3 : 0;
        int top = sideOffsetBooleans[1] ? lineWidth : 0;
        int right = sideOffsetBooleans[2] ? lineWidth/3 : 0;
