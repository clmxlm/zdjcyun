...
import com.zdjc.zdjcyun.mvp.presenter.impl.BeginPresenterImpl;
...
import com.zdjc.zdjcyun.mvp.viewmodel.IBeginModel;
...
public class BeginModel extends BaseModel<ActivityBeginBinding,BeginPresenterImpl> implements IBeginModel ,ViewPager.OnPageChangeListener{
...
//    private PDFRecycViewAdapter pdfRecycViewAdapter;
        ...
        new Thread() {
            public void run() {
                ...
                while (isRunning) {
                    ...
                    // 往下跳一位
                    ((BeginActivity) UI).runOnUiThread(() -> {
    ...
    private void initViews() {
        ...
        projectTypeRecycViewAdapter = new ProjectTypeRecycViewAdapter((BeginActivity) UI);
//        pdfRecycViewAdapter = new PDFRecycViewAdapter((BeginActivity) UI);
...
//		viewPager.setOffscreenPageLimit(1);// 左右各保留几个对象
        ...
        mBinder.includeLeft.rlLogout.setOnClickListener(v -> {
            if (!"".equals(token)){
                ...
                map1.put("dd","dd");
                mControl.loginOut(BeginModel.this, map1,3);
            }else {
                ((BeginActivity)UI).intent2Activity(LoginActivity.class);
                drawerLayout.closeDrawer(GravityCompat.START);
                ((BeginActivity)UI).finish();
            }
        });
...
//        pdfRecyclerView.setAdapter(pdfRecycViewAdapter);
        ...
        projectTypeRecycViewAdapter.setOnItemClickListener((view, position) -> {
            if ("".equals(PreferenceUtils.getString(getContext(),"token"))){
                ...
                ((BeginActivity) UI).intent2Activity(ProjectListActivity.class);
