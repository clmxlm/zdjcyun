...
import com.zdjc.zdjcyun.mvp.presenter.impl.DataQueryPresenterImpl;
import com.zdjc.zdjcyun.mvp.ui.activities.DataComparisonActivity;
import com.zdjc.zdjcyun.mvp.ui.adapter.ImgDotsAdapter;
import com.zdjc.zdjcyun.mvp.ui.adapter.MonitorPointRightRecycViewAdapter;
import com.zdjc.zdjcyun.mvp.ui.adapter.MonitorTypeLeftRecycViewAdapter;
import com.zdjc.zdjcyun.mvp.ui.adapter.WarningRecycViewAdapter;
import com.zdjc.zdjcyun.mvp.viewmodel.IDataQueryModel;
...
public class DataQueryModel extends BaseModel<FragmentDataQueryBinding, DataQueryPresenterImpl> implements IDataQueryModel, ViewPager.OnPageChangeListener {
    ...
    private ArrayList<SensorDataEntity.DataBean.CommonDataVOBean> commonDataVOBean = new ArrayList<>();
    private ArrayList<GPSNormEntity.DataBean.CommonDataVOBean> gpsCommonDataVOBean = new ArrayList<>();
    ...
    private MonitorTypeLeftRecycViewAdapter monitorTypeLeftRecycViewAdapter;
    private MonitorPointRightRecycViewAdapter monitorPointRightRecycViewAdapter;
    private ArrayList<MonitorTypePointNameEntity.DataBean> monitorList;
    ...
    private void intView() {
        ...
        mBinder.segmentTabLayoutMonitor.setOnTabSelectListener(new OnTabSelectListener() {
            ...
            public void onTabSelect(int position) {
                ...
                if (xValue1.length==0){
                    ...
                    curvelData(xValue1,commonDataVOBean);
                }
            }
            @Override
        ...
        RecyclerView rvIndex = mBinder.includeQueryLeft.rvIndex;
        monitorTypeLeftRecycViewAdapter = new MonitorTypeLeftRecycViewAdapter(getContext());
        GridLayoutManager manager = new GridLayoutManager(getContext(), 1);
        manager.setOrientation(GridLayoutManager.VERTICAL);
        rvIndex.setLayoutManager(manager);
        rvIndex.setAdapter(monitorTypeLeftRecycViewAdapter);


        RecyclerView rvPoint = mBinder.includeQueryLeft.rvPoint;
        monitorPointRightRecycViewAdapter = new MonitorPointRightRecycViewAdapter(getContext());
        GridLayoutManager manager1 = new GridLayoutManager(getContext(), 1);
        manager.setOrientation(GridLayoutManager.VERTICAL);
        rvPoint.setLayoutManager(manager1);
        rvPoint.setAdapter(monitorPointRightRecycViewAdapter);

        mBinder.llLeft.setOnClickListener(v -> {
            mBinder.drawer.openDrawer(GravityCompat.START);
            Map<String, String> map = new HashMap<>(0);
            map.put("sectorId", id);
            mControl.getQueryMonitorTypePointName(DataQueryModel.this, map, 6);
            monitorTypeLeftRecycViewAdapter.getOnclickPosition(0);
            monitorTypeLeftRecycViewAdapter.notifyDataSetChanged();
        });

        monitorTypeLeftRecycViewAdapter.setOnItemClickListener((view, position) -> {
            if (null!=monitorList){
                monitorPosition = position;
                monitorPointRightRecycViewAdapter.setDataList(monitorList.get(position).getMonitorPointNumber());
                PreferenceUtils.putInt(getContext(),"monitorType",monitorList.get(position).getMonitorType());
                PreferenceUtils.putString(getContext(),"monitorTypeName",monitorList.get(position).getMonitorTypeName());
                monitorTypeLeftRecycViewAdapter.getOnclickPosition(position);
                monitorTypeLeftRecycViewAdapter.notifyDataSetChanged();
            }
        });

        monitorPointRightRecycViewAdapter.setOnItemClickListener((view, position) -> {
            String mpn = monitorList.get(monitorPosition).getMonitorPointNumber().get(position);
            PreferenceUtils.putString(getContext(),"monitorPointNumber",mpn);
            /**
             * * 请求不同指标下的不同数据对象tag
             */
            Map<String, String> mapType = new HashMap<>(0);
            mapType.put("monitorType", PreferenceUtils.getInt(getContext(),"monitorType")+"");
            mControl.getQueryMapType(DataQueryModel.this, mapType, 4);
    ...
    private void querySensorData() {
        ...
        if (mapType==SENSOR_TYPE){
            mControl.getQuerySensorData(DataQueryModel.this, map, 2);
            ...
            mControl.getQueryNormTwoSensorData(DataQueryModel.this, map, 2);
            ...
            mControl.getQueryNormThreeSensorData(DataQueryModel.this, map, 2);
            mBinder.segmentTabLayoutMonitor.setVisibility(View.GONE);
            mBinder.segmentTabLayoutTime.setVisibility(View.GONE);
        ...
        switch (tag) {
            case 1:
                ...
                for (DataMonitoringImagesEntity.DataBean dataBean : dataBeans) {
                    ...
                    for (DataMonitoringImagesEntity.DataBean.MonitorPointsBean monitorPointsBean : dataBean.getMonitorPoints()) {
                        ...
                        if (!"".equals(monitorPointsBean.getPicXaxis())&&!"".equals(monitorPointsBean.getPicYaxis())){
                            int widthPx=Integer.valueOf(monitorPointsBean.getPicXaxis().replace("px", ""));
                            int heightPx=Integer.valueOf(monitorPointsBean.getPicYaxis().replace("px", ""));
            ...
            case 2:
                if (mapType==SENSOR_TYPE){
                    ...
                    xTimeData.clear();
                    commonDataVOBean = sensorDataEntity.getCommonDataVO();
                    for (SensorDataEntity.DataBean.CommonDataVOBean commonDataVOsBean : sensorDataEntity.getCommonDataVO()) {
                        xTimeData.add(commonDataVOsBean.getCreateDate());
                    }
                    xValue1 = xTimeData.toArray(new String[xTimeData.size()]);
                    curvelData(xValue1, sensorDataEntity.getCommonDataVO());
                }else if (mapType==SENSOR_TYPE_TWO){
                    GPSNormEntity.DataBean gpsNormEntity = (GPSNormEntity.DataBean) bean;
                    xTimeData.clear();
                    gpsCommonDataVOBean = gpsNormEntity.getCommonDataVO();
                    for (GPSNormEntity.DataBean.CommonDataVOBean commonDataVOBean : gpsNormEntity.getCommonDataVO()) {
                        xTimeData.add(commonDataVOBean.getCreateDate());
                    }
                    xValue1 = xTimeData.toArray(new String[xTimeData.size()]);
                    curvelGPSData(xValue1, gpsCommonDataVOBean);
                }else if (mapType==SENSOR_TYPE_THREE){
                    ...
                    map.put("beginTime", startTimeS);
                    map.put("endTime", endTimeS);
//                    mControl.getQueryDeepData(DataQueryModel.this, map, 5);
                }
                break;
            case 3:
            ...
            case 4:
                ...
                map.put("monitorType", monitorType+"");
                map.put("monitorPointNumber", monitorPointNumber);
                mControl.getQueryTerminalAndSensor(DataQueryModel.this, map, 3);
                break;
            case 5:
                ...
                break;
            case 6:
                monitorList = (ArrayList<MonitorTypePointNameEntity.DataBean>) bean;
                monitorTypeLeftRecycViewAdapter.setDataList(monitorList);
                monitorPointRightRecycViewAdapter.setDataList(monitorList.get(0).getMonitorPointNumber());
                break;
            default:
                break;
        }
    ...
    private void request() {
        ...
        map.put("sectorId", id);
        map.put("imageType", 3 + "");
        mControl.getQueryImagesMonitorPoint(DataQueryModel.this, map, 1);
    }
...
//        imageNumber.setText(String.format("%d/%d", position + 1, imageUrls.size()));
     ...
     * 初始化LineChart
     */
    private void curvelData(String[] xValue,ArrayList<SensorDataEntity.DataBean.CommonDataVOBean> yData){
     ...
     * 初始化GPS位移LineChart
     */
    private void curvelGPSData(String[] xValue,ArrayList<GPSNormEntity.DataBean.CommonDataVOBean> yData){
     ...
     * 统一曲线图设值
     */
    private void setData(ArrayList<SensorDataEntity.DataBean.CommonDataVOBean> data1) {
...
//        }
     ...
     * 展示多条曲线。
     */
    private void showLineChart(ArrayList<GPSNormEntity.DataBean.CommonDataVOBean> yAxisValues, ArrayList<String> labels) {
